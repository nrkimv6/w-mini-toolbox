...
commit wrapper 호출 직전에는 대상 repo의 `.claude\hooks\grant-commit.ps1`을 확인한다. helper가 있으면 `user-prompt:<요약 30자 이상>` 또는 해당 workflow prefix(`done-archive:`, `merge-test:`, `manual-tasks:`)로 새 sentinel을 발급하고, helper가 없으면 commit을 막지 않고 `no-sentinel-hook: <path>` evidence를 남긴다.
