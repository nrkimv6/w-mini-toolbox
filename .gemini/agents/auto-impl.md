...
5. **커밋**
   ...
   - commit wrapper 호출 직전 대상 repo의 `.claude\hooks\grant-commit.ps1`을 확인한다. helper가 있으면 `user-prompt:<요약 30자 이상>` reason으로 새 sentinel을 발급하고, 없으면 `no-sentinel-hook: <path>` evidence를 남긴다.
...
# commit 직전 grant-commit.ps1 -Reason "user-prompt:<요약 30자 이상>" 실행
