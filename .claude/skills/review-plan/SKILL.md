...
### 0.5단계: plan 본문 mutation boundary

review-plan은 계획서를 실행 가능한 상태로 다듬는 도구이지, 검토 로그를 계획서 본문에 저장하는 도구가 아니다.

**계획서 본문에 남길 수 있는 변경:**
- 실제 실행 계약으로 읽히는 범위/비목표/선행조건/owner/surface/downstream sync/read-back 조건
- 구현자가 수행해야 하는 TODO, Phase, 기술적 고려사항, 완료 기준
- 상태 필드 전이(`초안` -> `검토완료`, `수정필요`, `보류`)처럼 lifecycle 자체를 나타내는 메타

**계획서 본문에 남기면 안 되는 변경:**
- `review-plan 재검토 기록`, `검토 결과`, `판정`, `누락`, `요구 수정 방향`, `expand 판정`, `차단 사유` 같은 review 수행 로그 섹션
- "review-plan이 판단했다", "재검토에서 실패했다", "결과표에 기록했다"처럼 도구 실행 과정을 설명하는 문장
- 결과표/Closeout Evidence/검토 근거에 들어갈 내용을 계획서 본문에 복사한 문장

보정이 필요하면 review 결론을 그대로 붙이지 말고, 계획서의 실제 계약 문장으로 변환한다. 예를 들어 mirror surface 문제가 발견되면 `review-plan 차단 기록`을 쓰지 않고 `범위: wtools primary에서 수정, receiver는 ff-only read-back만 수행`처럼 실행 범위로 고친다. 실행 계약으로 변환할 수 없는 실패 사유는 계획서에 쓰지 않고 결과표와 최종 응답에만 남긴다.
...
**계약 변경 영향 체크 (공통 정책):**
- scheduler, worker claim, persisted JSON shape, API response, DB migration 의미가 바뀌는 plan은 기존 active/enabled 데이터 invalidation 분류가 있어야 한다.
- invalidated 데이터가 있으면 repair, migration, disable, visible error 중 하나가 TODO/완료 기준에 있어야 한다.
- `N/A`는 근거 파일, DB/API read-back, representative fixture 중 하나가 같이 적힌 경우에만 허용한다.
- enabled active entity가 후보 0개 또는 silent no-op이 되는 테스트 기대는 stale fixture 후보로 지적한다.
...
- 이관 plan 헤더 필수 필드: `> 원본 plan: <project>/<plan-path>`, `> 요약:` (원본 fence todo 요약), `> surface 분류:`
- 이관 plan 파일명 규칙:
...
| # | 계획서 | 재검토 | time anchor | 로컬변경 | 연관 active plan | archive 참조 | 환경오염 | scope split | surface isolation | surface 분류 | scope-coverage | 모호어 | expand | 실패메타데이터 | 비고 |
|---|--------|--------|-------------|----------|------------------|-------------|---------|-------------|-------------------|-------------|----------------|--------|--------|----------------|------|
| 1 | {파일명} | 통과 | {implementation_start/execution_start/absolute_event_time/unclear/해당 없음} | {영향 없음/참조만/보정 반영} | {0-hit/중복 회피/선행관계} | {0-hit/참조 반영} | {해당 없음/경고: {패턴}/차단: {사유}} | {해당 없음/승인 있음/split-required/split-applied/수동 결정 필요/차단: CODEX_SCOPE_SPLIT_UNAPPROVED} | {해당 없음/단일 surface/split-required/split-applied/수동 결정 필요} | {공통 정책/모델별 메커니즘/분류 모호/누락: SURFACE_CLASSIFICATION_MISSING} | {완전 매칭/{N}건 미커버/해당 없음} | {0건/{N}건 (예: ...)} | {N}개 작업 | {카테고리/종료코드/처리결과} | — |
| 2 | {파일명} | 실패 | {implementation_start/execution_start/absolute_event_time/unclear/해당 없음} | {재검토 실패/보정 반영} | {충돌/0-hit} | {참조만/0-hit} | {해당 없음/경고: {패턴}/차단: {사유}} | {해당 없음/수동 결정 필요/차단: CODEX_SCOPE_SPLIT_UNAPPROVED} | {해당 없음/수동 결정 필요} | {누락: SURFACE_CLASSIFICATION_MISSING} | {완전 매칭/{N}건 미커버/해당 없음} | {0건/{N}건 (예: ...)} | — | {있음/없음} | {사유} |
...
- **검토 메타 본문 삽입 금지** — review-plan의 판정/누락/차단/결과표/closeout 내용은 최종 응답과 결과표에만 남긴다. 계획서에는 실행 범위, TODO, 기술적 고려사항, 완료 기준처럼 구현자가 따라야 하는 계약만 남긴다.
