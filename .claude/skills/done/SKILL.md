...
| **2.785 UI target marker evidence** | UI 데이터 표시 회귀(live logs/history/catch-up/rendering 등) plan에서 `T4` 또는 `T5-http_live` evidence row가 `완료`임 | 실제 target/deep-link URL, 대표 데이터 식별자, representative rendered marker, placeholder absence가 evidence text 또는 log_ref에 있음 | `T4_UI_TARGET_MARKER_MISSING` hard blocker. API read-back과 mock UI E2E만으로 상태 변경/archive/TODO→DONE 금지 | data display regression은 사용자가 보는 실제 화면 marker가 closeout source of truth |
...
**UI target marker contract:** UI 데이터 표시 회귀는 API read-back, filesystem log read-back, mock UI E2E가 모두 성공해도 archive 근거가 부족할 수 있다. 실제 target/deep-link URL에서 representative rendered marker와 placeholder absence를 확인하지 못하면 `T4_UI_TARGET_MARKER_MISSING`으로 hard blocker 분류한다.
