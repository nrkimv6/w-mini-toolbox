...
| **2.785 UI target marker evidence** | UI data display regression, rendered data, history row, table/list/card value, or product UI marker를 완료 근거로 주장 | 실제 `target/deep-link URL`, representative data identifier, `representative rendered marker`, `placeholder absence`가 evidence table 또는 auto-test-e2e DETAIL에 모두 존재 | `T4_UI_TARGET_MARKER_MISSING` hard blocker. API read-back과 mock UI E2E만으로 상태 변경/archive/TODO→DONE 금지 | T5-http_live와 UI marker는 서로 대체하지 않음 |
...
**UI target marker evidence contract:** UI data display regression은 실제 `target/deep-link URL`, representative data identifier, `representative rendered marker`, `placeholder absence`가 모두 있어야 한다. API read-back과 mock UI E2E만 있으면 `T4_UI_TARGET_MARKER_MISSING`으로 archive를 차단한다.
...
5. Claude Code가 `MANUAL_TASKS.md`를 생성/수정했다면 archive/TODO/DONE 후속 이동 전에 `manual-tasks:<plan-slug> <요약 30자 이상>` reason으로 commit sentinel을 새로 발급한 뒤 해당 exact path를 `D:\work\project\tools\common\commit.ps1 -Files`로 즉시 커밋하고 commit hash를 사용자에게 보이는 closeout evidence에 남긴다.
...
- [ ] 테스트가 통과했는가?
- [ ] 빌드가 성공했는가?
- [ ] DB schema 변경이 있다면 Phase DB-Direct evidence 3종(실행 SQL/명령 또는 자동 bootstrap/restart 경로, 존재 확인 쿼리, live API/runtime 결과)을 확보했는가?
