...
- 계약 변경 여부: {scheduler | worker claim | persisted JSON shape | API response | DB migration | N/A}
- 기존 active/enabled 데이터 영향: {invalidated | compatible | N/A + 근거}
- invalidated 데이터 처리: {repair | migration | disable | visible error | N/A + 근거}
- 완료 evidence: {representative fixture | post-merge live read-back | API/DB query | visible error assertion}
...
`T4-operational-merge` row의 `log_ref`는 반드시 ① runner path 키워드(`execute_merge`, `handle_merge_stage`, `_rebase_branch_onto_main`, `runner-level`, `merge command path` 중 하나), ② `merge_history_skipped`, ③ read-back 키워드(`metadata`, `log`, `read-back`, `merge_history.` 중 하나)를 **모두** 포함해야 한다. 3종 중 하나라도 누락 시 `OPERATIONAL_MERGE_E2E_MISSING`. SSOT: `common/tools/merge-test-contract.md`.
