...
- canonical publish 전 반복 작성이 필요하면 `.worktrees/drafts/plan/<slug>_draft.md` flat draft 파일을 사용한다. `.worktrees/drafts/plan/<session-id>/draft.md` 폴더 세션과 `metadata.json` + `draft.md` 형식은 신규 Claude plan authoring 입력으로 만들지 않는다.
- `<slug>_draft.md`에는 publish 대상 후보와 승인/미승인 구분 근거를 front matter 또는 본문 metadata로 남긴다. surface split 시 child 파일명은 기존 계약대로 대표 plan stem + `_todo-N.md`만 허용한다.
...
- `모델별 메커니즘` 변경: 해당 모델 surface receiver의 ff-only read-back만 수행하고 다른 모델 surface에 동일 문구 강제 금지. `Downstream Sync Phase`는 receiver 해당 surface만 대상으로 삽입한다.
...
5. 커밋 스크립트 실행: `docs: plan {주제}`
   - ⚠️ plans lineage 모드에선 `commit.ps1` 실행 후 push 단계가 필요하다. push 전에 `_path-rules.md`의 **plans push — 로컬 전용 분기** 규칙에 따라 upstream 존재 여부를 먼저 확인한다. upstream 있으면 push (대상은 literal `origin plans`가 아니라 현재 docs commit root가 추적하는 upstream). upstream 없으면(로컬 전용 모드) `plans: local-only mode, push skipped` evidence를 기록하고 그대로 진행한다. root `main`/일반 feature branch에서는 continue하지 않는다.
