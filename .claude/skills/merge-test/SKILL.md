...
Operational merge validation is also owned by `common/tools/merge-test-contract.md`. Plan-runner merge policy/runtime changes with detector seeds such as `merge_stage`, `_rebase_branch_onto_main`, `merge policy`, `branch preflight`, or `dev-runner merge` require a `T4-operational-merge` row. UI/HTTP `T4/T5 해당 없음` does not waive this runner lifecycle gate; missing evidence is `OPERATIONAL_MERGE_E2E_MISSING`. A completed `T4-operational-merge` row's `log_ref` must include all three: (1) a runner path keyword (`execute_merge`, `handle_merge_stage`, `_rebase_branch_onto_main`, `runner-level`, or `merge command path`); (2) `merge_history_skipped`; (3) a read-back keyword (`metadata`, `log`, `read-back`, or `merge_history.`). Missing any one of the three maps to `OPERATIONAL_MERGE_E2E_MISSING` even when the row is present.
