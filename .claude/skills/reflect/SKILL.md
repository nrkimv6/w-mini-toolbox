...
## PLAN_SPLIT_GATE: plan 분할 전 Read 의무

사용자가 reflect 대화에서 plan 분할을 요청하거나 `/reflect` 수행 중 후속 plan을 parent/child로 나눌 필요가 생기면, child 파일을 만들기 전에 `.claude/skills/plan/SKILL.md`를 먼저 Read하고 그 분할 게이트를 적용한다.

- 사용자 발화 예: "분할해", "나눠", "child로 빼", "todo로 쪼개".
- 주제 분할 금지, surface 분할 우선: 분할 기준을 새 주제 slug로 정하지 말고 `.agents`, `.claude`, `.gemini`, generated Gemini runtime 등 authoring surface가 섞였는지 먼저 분류한다.
- surface 분류가 필요하면 parent 또는 child 헤더에 `> surface 분류:`를 표시한 뒤 진행한다.
- child 파일명은 기존 대표 plan 파일명에 `_todo-N.md` suffix만 붙이는 규칙을 따른다.
...
- 사용자가 대화에서 `[$done] [$reflect]`처럼 여러 skill을 같은 턴에 명시하면 각 skill의 실행 여부와 evidence를 별도 row로 남긴다. Claude Code에서는 이 직접 다중 호출이 primary 시나리오다.
...
|-----------|-----------|-------|----------------|---------------|----------------------------------------------|------|
| `{사용자 교정 요약}` | `{누락 또는 오판}` | `{Q5|Q6|Q5/Q6}` | `{recovery field}` | `{preventive field}` | `{plan/new|attach_existing|report-only}` | `{로그/파일/명령 근거}` |

- legacy read-back marker(출력 형식 아님): `| 발화 요지 | 누락/오판 | Q분류 | 처리(plan/new, attach_existing, report-only) | 근거 |`

- `수습/복구 결과`(`recovery field`)에는 사후 실행 결과만 쓴다. 예: `이후 N개 완료`, `merge 완료`, `archive 완료`, `done 처리 완료`, 재개한 owner action의 결과.
- `재발방지 대책`(`preventive field`)에는 skill/guard/TC/routing 변경 또는 existing preventive guard read-back만 쓴다. 사후 실행 결과를 preventive field에 쓰지 않는다.
- preventive control을 실제 수정 범위로 포함하지 않는 active plan에는 `attach_existing`하지 않는다. archive-only/success-only attach 금지.
- `수습/복구 결과`가 성공이어도 `재발방지 대책`이 비어 있으면 Q6를 `해당 없음`으로 닫지 말고 `report-only`, `attach_existing`, `plan/new` 중 하나와 근거를 남긴다.
...
- 질문형 merge/done 누락 지적(`왜 머지 안 했지`, `왜 머지를 하지 않았지`, `왜 done 안 했지`)은 deterministic next owner 누락 escalation으로 분류한다. hard blocker가 없으면 원인 1줄을 남긴 뒤 같은 턴에서 `/merge-test` 또는 `/done` owner action을 재개하고, 결과는 `수습/복구 결과`에만 기록한다.
...
- `attach_existing`은 active plan이 해당 preventive control을 실제 수정 범위로 포함할 때만 허용한다. archive-only/success-only attach 금지: archive에만 있거나 성공 기록만 있는 항목은 existing 근거가 아니며, 재발 리스크가 남아 있으면 `plan/new` 또는 `must_plan`으로 판정한다.
- intervention ledger에서 `수습/복구 결과`는 plan 귀속 근거가 아니다. `재발방지 대책`이 skill/guard/TC/routing 변경 또는 existing preventive guard read-back으로 확인될 때만 `attach_existing` 또는 `report-only`가 가능하다.
...
| 발화 요지 | 누락/오판 | Q분류 | 수습/복구 결과 | 재발방지 대책 | 처리(plan/new, attach_existing, report-only) | 근거 |
|-----------|-----------|-------|----------------|---------------|----------------------------------------------|------|
| {요약} | {누락 또는 오판} | {Q5|Q6|Q5/Q6} | {recovery field} | {preventive field} | {plan/new|attach_existing|report-only} | {근거} |

사용자 escalation 처리:
| escalation evidence | 다시 확인한 범위 | 수습/복구 결과 | 재발방지 대책 | next owner action | 처리 |
|---------------------|------------------|----------------|---------------|-------------------|------|
| {재지시/질책 요약} | {plan/TODO/diff/status/read-back 범위} | {recovery field} | {preventive field} | {/merge-test|/done|없음|blocked} | {done|remaining targets|blocked} |
