...
Final preflight reads `common\tools\session-target-router.ps1 -Json` for the current reflect/session target ledger and uses the router JSON as the source of truth. The router JSON must include `decision=continue|blocked|final`, `declared`, `processed_this_turn`, `already_archived`, `blocked`, `remaining_executable`, `next_owner`, and when blocked, `blocker_code`. `decision=continue` is normal continuation, not failure, and not blocked; route to the next reflect target, `/review-plan`, or owner from `next_owner` instead of producing report-only final wording. `decision=blocked` is target-local blocker evidence with `blocker_code` and `next_owner`. Only `decision=final` allows final reflect closeout.
...
- `/reflect`가 계획서를 자동 생성하거나 기존 계획서를 child/TODO로 분할할 가능성이 있으면, 생성/분할 판단 전에 `D:\work\project\service\wtools\.agents\skills\plan\SKILL.md`를 먼저 읽고 `분할 진입 게이트`와 `surface isolation preflight`를 적용한다.
- 사용자가 "분할해", "child로 나눠", "TODO를 쪼개"처럼 명령형으로 plan 분할을 요청한 경우도 같은 `PLAN_SPLIT_GATE` 적용 대상이다.
- 주제 분할 금지, surface 분할 우선. surface split은 project/Phase split보다 먼저 판단하고, child 파일명은 원본 stem + `_todo-N.md` suffix만 허용한다.
- wtools 내부에서 `.claude/`는 `.agents/` mirror가 아니라 Claude primary surface다. `.agents`/`.claude`/`.gemini` 관계를 mirror 전제로 축소하지 말고 각 엔진 primary surface로 분류한다.
...
- 세션의 `declared`, `processed_this_turn`, `already_archived`, `ignored`, `blocked`, `remaining_executable` 6컬럼 count를 reflect 입력 요약에 남긴다.
- 레거시 요약 marker `declared/processed/excluded/remaining`도 read-back 호환 근거로 유지하되, 최종 집계는 위 6컬럼을 우선한다.
...
- `MANUAL_TASKS immediate commit`, `ignored target`, `count mismatch`는 같은 세션에서 함께 발생해도 intervention ledger에 별도 행으로 남긴다.
...
| 발화 요지 | 누락/오판 | Q분류 | 수습/복구 결과 | 재발방지 대책 | 처리(plan/new, attach_existing, report-only) | 근거 |
|-----------|-----------|-------|----------------|---------------|----------------------------------------------|------|
| `{사용자 교정 요약}` | `{누락 또는 오판}` | `{Q5|Q6|Q5/Q6}` | `{사후 실행 결과 또는 없음}` | `{preventive control 또는 없음}` | `{plan/new|attach_existing|report-only}` | `{로그/파일/명령 근거}` |

- legacy read-back marker(출력 형식 아님): `| 발화 요지 | 누락/오판 | Q분류 | 처리(plan/new, attach_existing, report-only) | 근거 |`
...
- 사용자 escalation evidence가 있으면 report-only 출력 전에 active plan 귀속 여부와 신규 plan 필요성을 반드시 결과표에 남긴다. 결과표 컬럼은 최소 `escalation`, `active_plan_match`, `classification`, `owner_gap`, `수습/복구 결과`, `재발방지 대책`, `new_plan_required`, `next_owner`를 포함한다.
- automatic pipeline 재입력 escalation(예: `왜 머지 안해`, `수동으로 돌려야`, `자동 개발 파이프라인 맞아?`, 사용자가 이미 `진행해`/`해`를 말했는데 같은 action 재입력을 요구받은 경우)은 active plan이 없으면 `plan/new`로 승격한다.
...
- `ignored_dirty` false blocker escalation은 `merge-test-cleanup.ps1 -Json` read-back(`cleanup_ready`, `mutation_ready`, `hard_blockers`, `warnings`, `ignored_dirty_paths`)을 사용자 개입 ledger 근거로 남긴다. helper-first 재발 방지 조항이 이미 반영되어 있고 잔여 mutation owner가 없을 때만 `report-only`; 근거가 없거나 재발 방지 조항이 없으면 `must_plan`이다.
- compaction 후 재개 지연으로 escalation이 발생한 경우, 관련 SKILL.md에 `compaction resume gate` 조항이 없으면 `must_plan` 강제다.

### 수습/복구 결과와 재발방지 대책 분리 계약

- reflect 결과에서 `수습/복구 결과`와 `재발방지 대책`은 반드시 별도 필드로 분리한다.
- `수습/복구 결과`에는 누락 발견 이후 실제로 수행한 사후 실행 결과만 적는다. 예: `이후 N개 완료`, `merge 완료`, `archive 완료`, `done 처리 완료`, read-back 재실행 결과.
- `재발방지 대책`에는 skill/guard/TC/routing 변경 또는 existing preventive guard read-back만 적는다. 기존 guard read-back은 관련 SKILL.md/TC/helper가 같은 행동을 차단하는 문구나 검증 결과를 확인한 경우에만 허용한다.
- 사후 실행 성공, archive-only 기록, success-only read-back은 `재발방지 대책`이 아니다. 이런 항목은 `수습/복구 결과`에만 기록하고, preventive control이 없으면 `report-only` 또는 `attach_existing`으로 축소하지 않는다.
- `왜 머지 안 했지`, `왜 머지를 하지 않았지`, `왜 done 안 했지`는 deterministic next owner 누락 escalation이다. hard blocker가 없으면 원인을 1줄로 기록한 뒤 같은 턴에서 `/merge-test` 또는 `/done` owner action을 재개한다. hard blocker가 있으면 blocker와 필요한 owner step을 기록한다.
- `attach_existing`은 active plan이 해당 preventive control을 실제 수정 범위로 포함할 때만 허용한다. archive-only attach, success-only attach, 이미 끝난 결과만 담은 plan에 대한 attach는 금지한다.
...
- T4/T5 완료 row가 source-contract-only, DOM-only, zero-selector collect-only, selector/action/assertion 누락, 또는 worker/scheduler registration/readiness 누락으로 보이면 "테스트 추가 제안"이 아니라 "완료 게이트 차단" finding으로 기록하고 canonical parser blocker 보강 plan을 생성/attach한다.
...
- 사용자가 자동 파이프라인을 수동으로 다시 돌려야 했다는 escalation(예: `왜 머지 안해`, `왜 머지 안 했지`, `왜 머지를 하지 않았지`, `왜 done 안 했지`, `수동으로 돌려야`)은 Q5/Q6 `must_plan` 후보다. 단, deterministic next owner가 명확하고 hard blocker가 없으면 원인 1줄을 남긴 뒤 같은 턴에서 `/merge-test` 또는 `/done` owner action을 먼저 재개한다.
- deterministic next owner step이 있는데 사용자 감시/재입력에 의존해 `/merge-test`, `/done`, `/reflect`, receiver push/read-back을 이어가지 않은 경우는 Q6 교정 후보이며, active plan에 귀속되지 않으면 `plan/new`로 승격한다.
...
- 신규 plan 생성/자동 분할/사용자 명령형 분할 경로에서는 위 `PLAN_SPLIT_GATE`에 따라 `.agents/skills/plan/SKILL.md`를 먼저 읽는다.
...
| 발화 요지 | 누락/오판 | Q분류 | 수습/복구 결과 | 재발방지 대책 | 처리(plan/new, attach_existing, report-only) | 근거 |
|-----------|-----------|-------|----------------|---------------|----------------------------------------------|------|
| {요약} | {누락 또는 오판} | {Q5|Q6|Q5/Q6} | {사후 실행 결과 또는 없음} | {preventive control 또는 없음} | {plan/new|attach_existing|report-only} | {근거} |

사용자 escalation 처리:
| escalation evidence | 다시 확인한 범위 | 수습/복구 결과 | 재발방지 대책 | next owner action | 처리 |
|---------------------|------------------|----------------|---------------|-------------------|------|
| {재지시/질책 요약} | {plan/TODO/diff/status/read-back 범위} | {사후 실행 결과 또는 없음} | {preventive control 또는 없음} | {/merge-test|/done|없음|blocked} | {done|remaining targets|blocked} |
