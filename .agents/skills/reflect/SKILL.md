...
- 사용자 escalation evidence가 있으면 report-only 출력 전에 active plan 귀속 여부와 신규 plan 필요성을 반드시 결과표에 남긴다. 결과표 컬럼은 최소 `escalation`, `active_plan_match`, `classification`, `owner_gap`, `new_plan_required`, `next_owner`를 포함한다.
