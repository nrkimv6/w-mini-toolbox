...
`/merge-test`가 `commit.ps1` 또는 `commit.sh`를 호출해야 하는 모든 경로는 commit wrapper 직전에 대상 repo의 `.claude\hooks\grant-commit.ps1`을 확인한다.

- helper가 있으면 매 commit마다 새 sentinel을 발급한다. pending merge commit, docs cleanup commit, post-merge repair commit은 서로 토큰을 재사용하지 않는다.
- helper가 없으면 commit을 막지 않고 transcript에 `no-sentinel-hook: <path>` evidence를 남긴다.
- helper 실패, reason prefix 누락, 30자 미만 reason 거부는 hard stop이며 commit wrapper를 호출하지 않는다.
- staged ownership과 protected staged path 검증을 통과한 뒤, commit wrapper 호출 직전에 발급한다.
- 발급 reason과 commit 결과를 merge-test evidence에서 분리한다.

허용 reason prefix:

| 경로 | reason |
|------|--------|
| 일반 merge commit | `merge-test:<branch> merge completion <요약>` |
| commit-ready pending merge resolve | `merge-test:resolve:<branch> pending merge completion <요약>` |
...
**CDP fallback artifact hygiene (필수)**:
- CDP fallback이 profile, screenshot, DOM JSON, trace 같은 evidence 산출물을 만들면 기본 disposable 위치는 repo 밖 `$env:TEMP\codex\<repo>\<plan-slug>\<timestamp>\`다.
- repo 안에 증거를 남겨야 할 때만 ignored evidence root `.tmp/codex/<plan-slug>/<timestamp>/`를 사용한다. root `tmp/`, root `temp/`, root 직하위 evidence 파일/디렉터리 생성은 금지다.
- CDP fallback 이후 final 직전 `git status --short --untracked-files=all`로 root untracked를 다시 확인한다. repo-local artifact guard가 있으면(예: `scripts\diagnostics\check-root-artifacts.ps1`) 함께 실행한다.
`sync_gemini_surfaces.py --check`, pytest, Pester 같은 non-live 검증 게이트가 실패했다가 같은 턴에서 복구된 경우에도 T4/T5와 동일한 `Recovered validation ledger`를 작성한다. 필수 컬럼은 `stage`, `failed_command`, `exit_code`, `failure_class`, `recovery_action`, `recovered_command`, `recovered_result`이며, 최종 통과 summary가 첫 실패 row를 생략하면 `non_live_recovery_evidence_missing`으로 취급한다. `failure_class` 허용값: `test_fixture_stale`(sync marker drift, fixture mismatch), `environment_failure`(CRLF/regex 불일치, config 부재), `contract_regression`. `test_fixture_stale` 또는 `environment_failure`이고 `blocks_archive=false`, `blocks_other_targets=false`이면 warning-success path다. non-live recovery ledger 예시: `sync_gemini_surfaces.py --check exit=1 → marker sync 실행 → check OK(exit=0)`, `Invoke-Pester exit=1 → CRLF/regex 보정 → 12/15 passed`.
...
- unmerged marker가 없고 `MERGE_HEAD`만 남아 있으면 commit-ready pending merge로 분류한다. 현재 owner의 merge임을 확인할 수 있고 staged 구현성/mirror path가 없을 때만 직접 `git commit`을 호출하지 말고 `merge-test:resolve:<branch>` reason으로 commit sentinel을 발급한 뒤 `D:\work\project\tools\common\commit.ps1`로 merge commit을 마무리하고 1단계로 계속한다. staged `.agents/.claude/.gemini`가 있으면 owner 여부와 무관하게 root에서 닫지 않는다.
...
if (Test-Path $mergeHead) {
  ...
  Write-Warning "If this merge belongs to the current owner and has no protected staged paths, grant a merge-test:resolve:<branch> commit sentinel, then finish it via D:\work\project\tools\common\commit.ps1. If staged .agents/.claude/.gemini exists, preserve/abort and require upstream sync regeneration or remote fast-forward receive evidence."
}
...
- Edit 후 plans lineage worktree(`.worktrees/plans` 또는 `origin/plans` descendant sync worktree)에서 `Resolve-DocsCommitRoot` 반환 cwd로 이동하고 `Resolve-DocsCommitCandidates` 반환 파일만 `git add`한 뒤 `merge-test:docs:<branch>` reason으로 commit sentinel을 발급하고 `D:\work\project\tools\common\commit.ps1`로 `"chore: {slug} 머지 완료 기록"` 커밋을 수행한다. push는 literal `origin plans`가 아니라 현재 docs commit root가 추적하는 upstream으로만 진행하고, root `main`/일반 feature branch면 중단한다.
...
- `> worktree-owner:` 없으면(레거시): `{plan경로}/**/*.md`에서 동일 branch/worktree로 부모 역추적. 불일치 시 중단. 일치 시 `> worktree-owner:` 보강 기록 + `merge-test:docs:<branch>` reason으로 commit sentinel을 발급한 뒤 즉시 커밋(`commit "chore: worktree-owner 기록"`).
...
| UI 데이터 표시 회귀 + API read-back + mock UI E2E 조합 | 불충분 | 실제 target/deep-link URL에서 representative rendered marker를 Browser/Playwright/CDP로 확인하지 않으면 `T4_UI_TARGET_MARKER_MISSING` |
...
- UI 데이터 표시 회귀(예: live logs, history row, catch-up rendering)는 API live read-back과 mock UI E2E가 모두 성공해도 T4 완료가 아니다. 실제 target/deep-link URL, 대표 데이터 식별자, 화면에 렌더된 representative marker, 빈/재시도 placeholder absence를 evidence table에 남긴 뒤에만 완료로 올린다.
...
- UI 데이터 표시 회귀에서 `T4`/`T5-http_live` row가 `완료`여도 evidence text에 실제 target/deep-link URL, representative rendered marker, placeholder absence가 없고 API read-back 또는 mock UI만 있으면 완료 evidence로 쓰지 않는다. `T4_UI_TARGET_MARKER_MISSING` blocker_code로 `contract_regression` 처리한다.
...
- 커밋은 `commit "message"` 스크립트만 사용하고, `git commit` 직접 실행을 금지한다. commit wrapper 직전에는 `/done`의 sentinel contract에 따라 `done-archive:<plan-slug> <요약 30자 이상>` 또는 `manual-tasks:<plan-slug> <요약 30자 이상>` reason으로 새 sentinel을 발급한다.
