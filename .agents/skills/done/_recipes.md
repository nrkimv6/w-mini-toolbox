...
agent가 파일을 수정/생성/archive helper 호출할 때마다 추가:
...
$TouchedPaths.Add("docs/archive/2026-xx-xx_foo.md") # archive helper success 후
```
...
| archive helper success | 원본 path + archive destination path |
...
Codex `/done`에서 archive mutation의 정상 경로는 helper 호출이다. helper는 `git mv`만 사용하고 write+unlink fallback을 하지 않으며, JSON으로 staged source deletion + archive destination pair를 보고한다.

```powershell
$repoRoot = "D:\work\project\service\wtools\.worktrees\plans"
$planFile = Join-Path $repoRoot "docs\plan\YYYY-MM-DD_{주제}_todo.md"
$archiveFile = Join-Path $repoRoot "docs\archive\YYYY-MM-DD_{주제}_todo.md"

$jsonText = & "D:\work\project\service\wtools\common\tools\archive-plan.ps1" `
  -PlanFile $planFile `
...
if ($result.tool -ne "archive-plan") {
  throw "archive-plan helper JSON payload not found"
}
if ($result.status -ne "success" -or -not $result.archive_pair_ok) {
  throw "archive helper failed: $($result.error_code) $($result.reason)"
}
if ($result.half_archived_state) {
  throw "HALF_ARCHIVED_STATE: active source and archive destination both exist"
}

$Expected = @($result.expected_paths)
$Staged = @($result.staged_paths)
Write-Host "archive helper ok: reason=$($result.reason) expected=$($Expected -join ',') staged=$($Staged -join ',')"
```

Success 판정:

- `status == "success"` 그리고 `archive_pair_ok == true`
- real move: `expected_paths`가 active source와 archive destination을 모두 포함하고, `staged_paths` read-back에 두 path가 보존됨
- resume no-op: `reason == "already_archived_resume"`이면 active source 부재 + archive destination 존재를 helper가 확인한 경우에만 성공으로 취급
...
Test-Path -LiteralPath $planFile
Test-Path -LiteralPath $archiveFile
```

위 명령은 read-only 진단이다. helper 실패 후 Codex가 직접 `git mv`, `Move-Item`, `Remove-Item`, `git add`, commit wrapper를 실행해 복구하지 않는다.

### 마지막 fallback — helper 미설치 환경의 직접 `git mv`

아래 절차는 `common\tools\archive-plan.ps1`가 없는 downstream 환경에서만 사용한다. wtools/common-tools 환경에서는 helper 부재 또는 실패를 blocker로 보고하고 직접 fallback을 사용하지 않는다.
...
### orphan 미도입 프로젝트 — helper 미설치 fallback
