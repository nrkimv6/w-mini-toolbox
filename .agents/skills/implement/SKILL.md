...
`/implement`는 구현 leaf 실행, 테스트, plan progress update, 구현 커밋까지만 소유한다. continuation trigger routing, same-turn owner chaining, compaction resume, `/merge-test`/`/done` 선택은 `.agents/skills/continue-plan/SKILL.md`가 소유한다.

Worker completion must leave a tail handoff block in the progress update:
...
If `remaining_leaf=0`, do not choose `/merge-test` or `/done` here. Hand off to `/continue-plan`.
