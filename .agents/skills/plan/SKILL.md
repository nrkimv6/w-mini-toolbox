...
- plan/TODO/DONE 수정 직후 direct invocation closeout은 `common/tools/plan-docs-closeout.ps1 -RepoRoot <docs-commit-root> -Message <message> -TouchedFiles <paths>`를 우선 사용한다. 이 helper는 `docs-dirty-guard Commit` 다음 `End`를 같은 closeout step에서 실행해, `Commit` 또는 `End` 누락을 허용하지 않는다.
- helper를 사용할 수 없는 shell surface에서만 fallback으로 touched exact path를 `-TouchedFiles`/`--touched-files`에 전달해 `docs-dirty-guard Commit`을 호출한 뒤 즉시 `End`를 호출한다. PowerShell Commit 경로는 `D:\work\project\tools\common\commit.ps1 -Files`를 경유한다.
...
wtools 원본 skill/agent/common-doc 변경이 하위 프로젝트 canonical, mirror, generated, 또는 primary runtime surface를 통해 소비되는 계획서라면, TODO에 `Downstream Sync Phase`를 반드시 넣는다. wtools 내부의 `.agents`, `.claude`, `.gemini` surface를 하나의 mirror 묶음으로 취급하지 않고 각 surface 역할에 맞춰 evidence를 남긴다.
...
- 트리거: `.agents/skills`, `.claude/skills`, `.agents/agents`, `.claude/agents`, common-doc plan/archive/TODO 계약처럼 wtools 원본 변경이 downstream canonical, mirror, generated, 또는 primary runtime surface로 반영되어야 하는 경우.
- downstream 예시: monitor-page `.agents/.claude` mirror, 프로젝트별 local skill mirror, Gemini headless primary surface(`common/tools/plan-runner/gemini-agents/*.md`), interactive convenience command(`.gemini/commands/*.toml`).
...
`6단계: 안내`는 closeout helper 또는 fallback `Commit` + `End`가 모두 성공한 이후에만 출력한다. Commit evidence 또는 `End status=ok` evidence가 없으면 성공 응답을 출력하지 않는다.
...
2. 이번 direct invocation이 실제로 수정한 touched exact paths를 산출한다. plans 워크트리에서는 `Resolve-DocsCommitCandidates` 반환값만 사용한다.
3. `common/tools/plan-docs-closeout.ps1 -RepoRoot <docs-commit-root> -Message "docs: plan {주제}" -TouchedFiles <exact paths>`를 실행한다.
4. helper 사용 불가 시에만 `docs-dirty-guard.ps1 -Mode Commit ... -TouchedFiles <exact paths>` 실행 직후 `docs-dirty-guard.ps1 -Mode End ...`를 실행한다.
5. `Commit` 출력이 `status=committed`이면 commit hash를 기록한다. `status=no-op`이면 no-op evidence를 기록한다.
6. `docs-dirty-guard: mode=End status=ok` 출력이 없으면 성공 응답 금지다.
7. `git diff --cached --name-status`와 `git status --porcelain` 재확인
...
8. closeout 성공 여부 확인: 실패 시 원인 보고 후 재시도하고, 성공 전까지 6단계로 진행하지 않는다.
9. 커밋 성공 후 plans push 단계: `_path-rules.md`의 **plans push — 로컬 전용 분기** 규칙에 따라 upstream 확인 후 push하거나 skip evidence를 기록한다.
