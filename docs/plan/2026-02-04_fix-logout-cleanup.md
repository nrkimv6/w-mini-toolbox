...
- [x] `cleanup()` 메서드를 신규 생성 (lines 119-137)
- [x] cleanup 안에서 사용자 옵션(`autoClearAfterCopy`, `autoClearAfterDownload`, `autoSaveInput` 등)을 기본값으로 리셋
- [x] cleanup 안에서 관련 localStorage 키를 삭제 (`clearAllSettings()` 호출)
- [x] store의 export에 cleanup 추가

### B. auth.svelte.ts에 SIGNED_OUT 핸들러 추가

**파일**: `src/lib/stores/auth.svelte.ts`

- [x] `onAuthStateChange` 콜백에서 이벤트 타입 파라미터를 받도록 수정 (line 31)
- [x] `event === 'SIGNED_OUT'` 분기를 추가 (lines 32-38)
- [x] SIGNED_OUT 분기 안에서 html-to-md 스토어의 `cleanup()` 호출 (line 34)
